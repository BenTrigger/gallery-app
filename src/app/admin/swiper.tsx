...
export default function SwiperAdmin() {
  ...
  useEffect(() => {
    fetch("/api/gallery-swiper").then(res => res.json()).then(setImages);
  }, []);
  ...
  const handleUpload = async () => {
    if (selectedFile) {
      ...
      await fetch("/api/gallery-swiper", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ src: `/uploads/${selectedFile.name}`, title: editData.title || selectedFile.name, description: editData.description || '', category: editData.category || 'main', type: 'image' })
      });
